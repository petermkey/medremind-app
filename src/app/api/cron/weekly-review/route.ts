...
//
// Trigger/work are decoupled: the per-user LLM generation loop far exceeds
// cron-job.org's fixed 30s client timeout, so the route returns 200 as soon as
// the trigger is validated and runs the work in after() (maxDuration 300). The
// Sentry check-in is the real success/failure monitor, not cron-job.org.
...
export async function GET(request: NextRequest) {
  ...
  after(() => runWeeklyReview(checkInId));

  return NextResponse.json({ triggered: true });
}

async function runWeeklyReview(checkInId: string): Promise<void> {
  try {
    ...
    Sentry.captureException(err, { tags: { route: 'cron/weekly-review', stage: 'after' } });
    Sentry.captureCheckIn({ checkInId, monitorSlug: MONITOR_SLUG, status: 'error' });
  }
}
