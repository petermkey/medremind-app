  ...
  return (
    <div className="flex flex-col h-full">
      <div className="px-5 pt-4 pb-3 flex-shrink-0">
        ...
        <h1 className="text-xl font-semibold tracking-[-0.02em] text-[#e8e6e1] mb-4">Medications</h1>
      ...
      <div className="flex-1 overflow-y-auto px-5 pb-6">
        ...
        {tab === 'catalogue' && (
          <>
            <p className="text-xs text-[#9b978f] mb-4">Reference database of {drugs.length} common medications and supplements.</p>
            {filteredDrugs.map(drug => (
              <div key={drug.id} className="bg-[#14171b] border border-[rgba(255,255,255,0.08)] rounded-xl p-4 mb-2.5">
                <div className="flex items-start justify-between gap-2">
                  <div>
                    <div className="text-sm font-bold text-[#e8e6e1]">{drug.name}</div>
                    {drug.genericName && drug.genericName !== drug.name && (
                      <div className="text-xs text-[#9b978f] mt-0.5">{drug.genericName}</div>
                    )}
                    {drug.commonDoses && drug.commonDoses.length > 0 && (
                      <div className="text-xs text-[#9b978f] mt-1">
            ...
            {!search && drugs.length > 30 && (
              <p className="text-xs text-[#9b978f] text-center py-3">Showing 30 of {drugs.length}. Search to filter.</p>
            )}
          </>
        )}
      </div>
