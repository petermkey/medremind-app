...
export function assertSafeCorrelationInsightText(text: string): void {
  if (DIRECT_MEDICATION_CHANGE.test(text)) {
    throw new Error('Direct medication-change language is not allowed');
  }
}

export function assertSafeCorrelationInsightCardText(title: string, body: string): void {
  assertSafeCorrelationInsightText(title);
  assertSafeCorrelationInsightText(body);
}
