  ...
  return {
    ...
    payload: validateWeeklyReviewPayload({
      ...
      highlights: [
        ...
        `Doses on plan: ${aggregate.adherence.adherencePct ?? 0}%`,
        `Average calories: ${aggregate.food?.weekAvg.kcal ?? 0} kcal/day`,
      ],
      eatingPatterns: [{ title: 'Mock pattern', detail: 'Generated by the mock provider for local testing.' }],
      stackAdherence: { summary: `Took ${aggregate.adherence.takenCount} of ${aggregate.adherence.plannedCount} doses.` },
      ouraLinkage: [],
      actions: [
        { title: 'Mock action 1', detail: 'Check the review rendering on the Progress page.' },
        { title: 'Mock action 2', detail: 'Check deduplication of a repeated cron run.' },
      ],
    }),
  };
}
