...
### What is built
- **OAuth2** flow (`src/lib/oura/oauth.ts`, `client.ts`): scopes `email personal daily heartrate tag workout session spo2 heart_health stress ring_configuration` (widened 2026-07-13 for heart_health/stress, 2026-07-14 for ring_configuration — see §2 correction below); tokens encrypted at rest (`tokenCrypto.ts`, `OURA_TOKEN_ENCRYPTION_KEY`), refresh-token exchange implemented.
