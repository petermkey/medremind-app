...
- The Display name field shows the signed-in account's real name — screenshots/specs derived from this page should redact or genericize it before sharing outside the team.
...
| 7 | 🟢 Low | `/onboarding` vs `/app/settings` | Onboarding pre-fills Age range "31–50" while the account's saved profile is "51–70" | ✅ Fixed 2026-07-15 — root cause was the persisted Zustand store hydrating from localStorage *after* the component's `useState` initializers ran, so `profile` was still `undefined` at mount. Added a one-time backfill `useEffect` that syncs `name`/`ageRange` once `profile` becomes available, without clobbering anything already typed. Verified live: re-entering onboarding now correctly pre-fills the account's saved name and "51–70". |
